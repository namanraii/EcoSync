...
export default function OnboardingWizard(): JSX.Element {
  ...
  const handleNext = (): void => {
    if (currentStep < STEPS.length - 1) {
      ...
      if (announcement) {
        announcement.textContent = `Step ${currentStep + 2} of ${STEPS.length}: ${STEPS[currentStep + 1].title}`
      }
    } else {
      handleSubmit()
    }
  ...
  const handleBack = (): void => {
    if (currentStep > 0) {
      ...
      const announcement = document.getElementById('step-announcement')
      if (announcement) {
        announcement.textContent = `Step ${currentStep} of ${STEPS.length}: ${STEPS[currentStep - 1].title}`
      }
    }
  }
